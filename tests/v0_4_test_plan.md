...
- wait for a task to finish with `gt_join()`
- query task status
...
The old `gt_go()` and `gt_go_with_stack()` APIs must continue to work.

## API Contract Decisions

- A task handle remains valid until `gt_task_handle_release()` is called.
- `gt_join()` blocks the current green thread until the target task reaches DONE or CANCELLED.
- `gt_join()` outside a running green thread should return a documented error instead of blocking the OS thread.
...
- A task cannot join itself; this returns `GT_ERR_STATE`.
- Cancellation is cooperative. `gt_task_cancel()` sets a flag and wakes the task if it is sleeping or channel-waiting.
- A cancelled task must observe cancellation through `gt_task_cancelled()` or through interrupted blocking runtime calls.
...
- **TC-V04-REG-004**: `gt_go()` still creates detached tasks as before.
- **TC-V04-REG-005**: `gt_go_with_stack()` still honors custom stack sizes.
- **TC-V04-REG-006**: channels still wake senders/receivers correctly after handle support is added.

## 2. Handle Creation Tests

- **TC-HANDLE-CREATE-001**: `gt_go_handle(fn, arg)` returns a non-null handle for a valid task.
- **TC-HANDLE-CREATE-002**: `gt_go_handle(NULL, arg)` fails cleanly.
- **TC-HANDLE-CREATE-003**: `gt_go_handle_with_stack()` accepts valid custom stack sizes.
- **TC-HANDLE-CREATE-004**: `gt_go_handle_with_stack()` rejects too-small stack sizes.
...
- **TC-JOIN-008**: self-join returns `GT_ERR_STATE` and does not deadlock.
- **TC-JOIN-009**: `gt_join(NULL)` returns `GT_ERR_INVALID`.
- **TC-JOIN-010**: `gt_join()` outside a green thread returns the documented error.
...
- **TC-STATUS-009**: `gt_task_status(NULL, out)` returns `GT_ERR_INVALID`.
- **TC-STATUS-010**: `gt_task_status(handle, NULL)` returns `GT_ERR_INVALID`.

## 5. Cancellation Tests

- **TC-CANCEL-001**: cancelling a READY task marks it cancelled before it runs.
- **TC-CANCEL-002**: cancelling a RUNNING task sets the cancellation flag; task exits cooperatively after checking `gt_task_cancelled()`.
...
- **TC-CANCEL-007**: cancelling a null handle returns `GT_ERR_INVALID`.
...
- **TC-RELEASE-006**: detached tasks do not leak after `gt_run()` completes.
...
- **TC-SCHED-HANDLE-006**: if only impossible joins remain, `gt_run()` returns a documented deadlock error.
...
- **TC-MISUSE-HANDLE-003**: joining from outside a green thread is invalid unless final API explicitly supports a blocking owner-thread join.
